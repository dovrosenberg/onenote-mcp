...
const OUTLINE: Outline = { id: 'div:{aaa}{32}', dataId: '_default', left: 48, top: 120, width: 624 };
...
test('parseOutlines returns the top-level divs, in order, with their geometry', () => {
  ...
  assert.deepEqual(outlines, [
    OUTLINE,
    { id: 'div:{bbb}{113}', dataId: null, left: 723, top: 143, width: 624 },
  ]);
});
...
test('a page with no body element is scanned whole', () => {
  ...
  assert.deepEqual(outlines, [{ id: '', dataId: null, left: 10, top: 20, width: null }]);
});
...
test('no ink means no padding', () => {
  assert.equal(planInkClearance(OUTLINE, null, null), null);
});
...
test('ink beside the outline column is not in the way', () => {
  ...
  assert.equal(planInkClearance(OUTLINE, box(700, 30, 900, 500), null), null);
});

test('ink above the outline is not in the way', () => {
  assert.equal(planInkClearance(OUTLINE, box(50, 10, 500, 100), null), null);
});

test('ink hanging below the outline top is padded from that top', () => {
  const plan = planInkClearance(OUTLINE, box(45, 30, 533, 466), null);
...
test('an outline with no declared width is assumed to be Graph’s default width', () => {
  const noWidth: Outline = { ...OUTLINE, width: null };
...
test('the padding is breaks, then the fragment wrapped in the marker', () => {
  ...
  assert.match(html, /<div data-id="ink-clearance-490"><p>appended<\/p><\/div>$/);
});
